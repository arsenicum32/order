...
// https://s.codepen.io/Arsenicum32/debug/BwVmeJ/LDMmdVpWLYqk
...
function drawing(svg){
    ...
    //middle.delay(400).animate(1).opacity(1)

    //middle.delay(100).animate(interval).move(-(-MX+30), 0).scale(0, 1).opacity(0)
    ...
    for(var i=0;i<20;i++){
      ar.push(draw.rect(10,10))
    }
    for(var i=0;i<20;i++){
      ar[i].fill('white')
    }

    function g(min,max){return Math.floor(Math.random()*(max-min+1)+min);}

    front.delay(interval*.9).animate(interval*.1).opacity(1).after(_=>{
      lez.animate(120).scale(-1,1).loop(40)
      ...
      for(var i=0;i<20;i++){
        ...
        .delay(g(50,300))
        .animate(500)
        .transform({rotation: Math.floor(Math.random()*1000)})
        .move(270 + g(-100,100) , 550 + g(-100,100)).opacity(0)
      }

      frontKran.animate(interval*6).move(0,30).after(_=>{
        frontKran.animate(interval*6).move(0,0)
      })
